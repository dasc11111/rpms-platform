...
## Actualización (20/09/2026, continuación Fase 5-8 — verificación en vivo del motor NCRP147 diagnóstico)

Wiring completado en una sesión previa: se conectaron dos funciones nuevas en blindaje-app.tsx (calcularBarreraPrimariaDiagnosticoNCRP147Click y calcularBarreraSecundariaDiagnosticoNCRP147Click), usando las funciones ya existentes y verificadas en ncrp147-diagnostic-calc-engine.ts (Tablas 4.5/4.7, distribuciones de carga de trabajo, componentes de dispersión). Commits 44950d2 (wiring) y f485a11 (fix de build). Ambos botones ("Calcular barrera primaria/secundaria (NCRP147, Apéndice B/C)") aparecen en el Paso 7 cuando facility_type es diagnóstico.

En esta actualización se realizó la prueba funcional en vivo (navegador, extremo a extremo: datos → UI → cálculo → persistencia), pendiente desde el cierre de la sesión anterior. Metodología: antes de ejecutar la UI, se calculó a mano el resultado esperado usando los mismos coeficientes/fórmulas leídos directamente del código fuente (cero fabricación), y se comparó ese valor contra el resultado producido por la UI como criterio de aprobación.

Incidente registrado y resuelto durante la prueba: la pestaña del navegador usada para probar apuntaba a un dominio de despliegue Vercel obsoleto (preview URL de una sesión anterior), no al despliegue "Ready" más reciente del commit f485a11. Esto causó que los campos "Distribución de carga de trabajo" y "Componente de radiación secundaria" parecieran ausentes del formulario (falso positivo de "funcionalidad faltante"). Se confirmó el dominio correcto abriendo el detalle del despliegue en Vercel (lista de "Domains" de esa build específica) y se repitió la prueba en el dominio correcto (rpms-platform-git-feature-fa-8e5140-tmdiegosolis-1552s-projects.vercel.app), donde los campos sí existen y funcionan. No hubo ningún defecto real de código; queda como lección operativa para futuras pruebas en vivo.

Caso de prueba (proyecto "Test Paso 6 Verificación", facility_type=diagnóstico):
PIR "PIR-TEST-NCRP147" (distancia=2 m, factor de ocupación T=1, criterio de diseño = 0.1 mGy/semana kerma en aire, NCRP147_KERMA_AIRE); carga de trabajo en modo detallada (100 procedimientos/semana, 100 pacientes/semana); barrera primaria "B-TEST-PRIMARIA" (Hormigón, distribución "Sala Rx (piso u otras barreras)", U=0.25): resultado UI = B=3.077e-3, K(0)=32.5 mGy/semana, espesor requerido=7.03 cm, cálculo manual independiente (K1=5.2 mGy/paciente, N=100, U=0.25, d=2 m, P=0.1 mGy/semana) con mismos valores exactos; barrera secundaria "B-TEST-SECUNDARIA" (Hormigón, misma distribución, componente lateral): resultado UI = B=1.739e-1, K(0)=0.575 mGy/semana, espesor requerido=1.28 cm, cálculo manual independiente (K1=2.3e-2 mGy/paciente lateral) con mismos valores exactos.

Ambos registros se guardaron con result_status="revisar" (el sistema nunca marca "cumple" automáticamente, según diseño). Conclusión: el motor de cálculo NCRP147 diagnóstico queda verificado extremo a extremo con coincidencia exacta contra cálculo manual independiente, sin fabricación de datos.

Pendiente para continuar Fase 5-8: (1) barrera secundaria para aceleradores (Ecs. 2.7, 2.8 y regla de las dos fuentes de NCRP151); (2) laberintos/puertas y neutrones/gammas de captura para aceleradores aún sin wiring a la UI (funciones ya existen en ncrp151-laberintos-puertas-references.ts); (3) extender el selector de "Componente de radiación secundaria" y validaciones de unidad a otros criterios de diseño legado si el usuario lo solicita.

## Corrección (20/09/2026) y verificación en vivo: barrera secundaria para aceleradores (NCRP151)

Al revisar el código fuente actual de blindaje-app.tsx para continuar con el punto (1) de la lista "Pendiente" anterior (barrera secundaria para aceleradores), se encontró que la función calcularBarreraSecundariaAceleradorClick() y su botón "Calcular barrera secundaria (NCRP151, Ec. 2.7/2.8)" YA ESTÁN completamente implementados en el Paso 7 del wizard (facility_type=radioterapia), usando las funciones ya verificadas de ncrp151-acelerador-barreras-references.ts (calcularFactorTransmisionDispersionPaciente, calcularFactorTransmisionFuga, combinarBarreraSecundariaDosFuentes). La afirmación anterior de que esto estaba "pendiente" era incorrecta (documentación desactualizada respecto al código real); se corrige aquí conforme al principio de cero fabricación (S61): nunca se documenta como pendiente algo que ya existe, ni viceversa.

Evidencia: el proyecto de prueba "Test Paso 13 Medicina Nuclear" ya contenía una barrera secundaria guardada previamente ("B-02 Muro secundario prueba", espesor=84.6 cm, estado=revisar), y en esta sesión se repitió la prueba en vivo con un registro nuevo ("B-TEST-SEC-VERIF") usando el mismo PIR (T=0.5, P=0.0001 Sv/semana a 3 m), fuente de 6 MV, carga de trabajo 500 Gy/semana, material hormigón, ángulo de dispersión 90 grados (lateral) y dsca=1 m. Cálculo manual independiente (Ec. 2.7): Bps=8.451e-3, n=2.07 TVL, espesor dispersión=35.2 cm. Cálculo manual independiente (Ec. 2.8): BL=1.8e-3, n=2.74 TVL, espesor fuga=84.6 cm. Regla de las dos fuentes: diferencia entre espesores (49.4 cm) es mayor o igual a 1 TVL (29 cm), por lo que se usa directamente el espesor mayor sin sumar HVL (criterio USAR_MAYOR_SIN_SUMA). Espesor final = 84.6 cm. El resultado producido por la UI coincidió exactamente con este cálculo manual en ambos casos (B-02 y B-TEST-SEC-VERIF), confirmando que la implementación es correcta.

Estado real actualizado de los pendientes de Fase 5-8 para aceleradores: barrera primaria (NCRP151 Ec. 2.1-2.3) y barrera secundaria (NCRP151 Ec. 2.7/2.8, dispersión de paciente + fuga + regla de las dos fuentes) están COMPLETAS y verificadas en vivo. Sigue pendiente únicamente: wiring de laberintos/puertas y neutrones/gammas de captura para aceleradores a la UI (las funciones ya existen en ncrp151-laberintos-puertas-references.ts, Ecuaciones 2.5, 2.6, 2.9 a 2.22, pero ningún botón de cálculo las invoca todavía desde blindaje-app.tsx; el Paso 11 - Laberintos actualmente solo permite registrar datos manualmente, sin calculadora).
