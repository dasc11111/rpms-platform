 ...
 * Evidencia grafica y documental asociada a una prueba o al equipo
 * (seccion 31 del prompt maestro). El archivo en si se administra fuera
 * de este modulo (almacenamiento externo de blobs); aqui solo se
 * registra la referencia/URL del archivo y sus metadatos. No se realiza
 * carga de archivos desde esta pantalla.
 */
...
const EVIDENCE_TYPES = [
  ...
  { value: "certificado_fuente", label: "Certificado de fuente/patron" },
  { value: "informe_calibracion", label: "Informe de calibracion / servicio" },
...
export default function ActivimetroEvidenceApp({ equipment }: { equipment: Equipment[] }) {
  ...
  async function loadEvidence() {
    ...
    try {
      ...
      const res = await fetch("/api/quality-control/activimetro/evidence?" + params.toString());
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/quality-control/activimetro/evidence", {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Evidencia Grafica y Documental - Activimetro</h1>
        <p className="text-sm text-gray-500">
          Referencia y metadatos de evidencia asociada a una prueba o al equipo (seccion 31 del
          prompt maestro). El archivo se administra en almacenamiento externo; aqui solo se
          guarda su URL de referencia. No se realiza carga de archivos desde esta pantalla.
        </p>
      </div>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="space-y-2">
            {records.map((r) => {
              ...
              const isPdf = isPdfUrl(r.file_url);
              return (
