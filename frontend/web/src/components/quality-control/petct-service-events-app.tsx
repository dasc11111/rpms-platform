 ...
 * Gestion de eventos de servicio tecnico (seccion 26 del prompt de mejora):
...
export default function PetCtServiceEventsApp({ equipment }: { equipment: Equipment[] }) {
  ...
  async function loadEvents() {
    const url = filterEquipment
      ? `/api/quality-control/petct/service-events?equipmentId=${filterEquipment}`
      : "/api/quality-control/petct/service-events";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/quality-control/petct/service-events", {
        method: "POST",
  ...
  async function updateStatus(event: ServiceEvent, status: string) {
    await fetch("/api/quality-control/petct/service-events", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Eventos de Servicio Tecnico PET/CT</h1>
        <p className="text-sm text-gray-500">
          Modulo 4 - Fase I. Registro de intervenciones tecnicas (seccion 26 del prompt de mejora) y
          seguimiento de las pruebas de control de calidad que corresponde repetir tras cada intervencion.
          El sistema solo lista las pruebas requeridas segun el catalogo; no decide resultados clinicos.
        </p>
      </div>
          ...
          return (
            <div key={ev.id} className="border rounded-lg p-4 space-y-2 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold border ${STATUS_STYLES[ev.status] ?? ""}`}>
