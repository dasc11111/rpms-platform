    ...
    timer.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/contamination/suggestions?field=${encodeURIComponent(field)}&q=${encodeURIComponent(q)}`
        );
        if (!res.ok) return;
  ...
  return (
    <div className="relative" ref={boxRef}>
      <input
        ...
        className={className ?? "w-full rounded-md border border-border bg-background px-2.5 py-1.5 text-sm outline-none focus:border-accent"}
        onChange={(e) => {
