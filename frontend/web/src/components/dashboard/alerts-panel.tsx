...
const TONE: Record<string, string> = { critical: "text-danger bg-danger-subtle", high: "text-warning bg-warning-subtle", medium: "text-warning bg-warning-subtle", info: "text-info bg-info-subtle" };
      ...
      {ALERTS.map((a) => {
        ...
        return (
          <Link href={a.href} key={a.id} className="group flex items-start gap-2.5 rounded-md border border-border bg-surface p-2.5 hover:border-accent">
            <span className={cn("flex h-6 w-6 items-center justify-center rounded-md", TONE[a.severity])}>
