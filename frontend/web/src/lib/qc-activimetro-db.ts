 ...
 *   cada prueba, y alertar si existe retraso (requisito TECDOC-602 / prompt maestro).
 */
...
export async function ensureActivimetroQcTables() {
  ...
  // Migracion segura para bases de datos donde la tabla ya existia sin esta columna
  await sql`ALTER TABLE qc_activimetro_tolerances ADD COLUMN IF NOT EXISTS frequency_days INTEGER;`;
...
async function seedDefaultTolerances() {
  ...
  const rows: Array<{
      ...
      { test_type: "precision", parameter_name: "cv_percent", tolerance_percent: 5, num_readings_required: 10, frequency_days: 1, notes: null },
      { test_type: "exactitud", parameter_name: "percent_difference", tolerance_percent: 5, num_readings_required: 3, frequency_days: 365, notes: null },
      { test_type: "linealidad", parameter_name: "percent_difference", tolerance_percent: 10, num_readings_required: 5, frequency_days: 90, notes: "REVISAR CON FISICO MEDICO: frecuencia asumida trimestral (90 dias); ajustar segun protocolo institucional vigente" },
      { test_type: "respuesta_fondo", parameter_name: "measured_value", tolerance_percent: null, num_readings_required: 1, frequency_days: 1, notes: "REVISAR CON FISICO MEDICO: tolerancia absoluta segun fondo habitual del equipo" },
      { test_type: "geometria_volumen", parameter_name: "percent_difference", tolerance_percent: 10, num_readings_required: 1, frequency_days: 365, notes: "REVISAR CON FISICO MEDICO: frecuencia asumida anual; ajustar segun protocolo institucional vigente" },
        ];
          ...
          await sql`
                INSERT INTO qc_activimetro_tolerances
                        (test_type, parameter_name, tolerance_percent, reference_source, protocol_version, num_readings_required, frequency_days, notes)
                              VALUES
                                      (${row.test_type}, ${row.parameter_name}, ${row.tolerance_percent}, 'Documento QA Activimetro proporcionado por usuario', '1.0', ${row.num_readings_required}, ${row.frequency_days}, ${row.notes})
...
/**
 * Backfill de frequency_days para filas insertadas antes de que esta columna
 * existiera (por ejemplo, en despliegues previos de esta misma rama). Solo
 * completa valores NULL; nunca sobrescribe un valor ya definido por el OPR.
 */
async function backfillFrequencyDays() {
      const defaults: Record<ActivimetroTestType, number> = {
              precision: 1,
              exactitud: 365,
              linealidad: 90,
              respuesta_fondo: 1,
              geometria_volumen: 365,
      };

  for (const [testType, days] of Object.entries(defaults)) {
          await sql`
                UPDATE qc_activimetro_tolerances
                      SET frequency_days = ${days}
                            WHERE test_type = ${testType} AND frequency_days IS NULL
                                `;
  }
}
