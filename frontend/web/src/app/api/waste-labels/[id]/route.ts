...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const lastMeasurement = measurements.length ? measurements[measurements.length - 1] : null;
      ...
      let fechaUltimaMedicion: string | null = lastMeasurement ? toISODateString(lastMeasurement.fecha) : null;
  ...
  if (!lastMeasurement && label.actividad_superficial_inicial_bq_cm2 !== null && label.fecha_medicion_superficial) {
          ...
          fechaUltimaMedicion = toISODateString(label.fecha_medicion_superficial);
  }
  ...
  if (existing.room_release_id && (body.waste_type !== undefined || body.waste_type_other !== undefined)) {
          ...
          if (resolvedKey) {
                    ...
                    if (found && release) {
                                ...
                                fechaMedicionSuperficial = toISODateString(release.release_date);
                                const fechaMedStr = toISODateString(release.release_date);
                                if (fechaMedStr) {
