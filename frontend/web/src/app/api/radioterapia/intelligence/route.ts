...
const EFICACIA_LABELS: Record<string, string> = {
    EFECTIVA: "Efectiva",
...
const allCauseEventsRes = await Promise.all([
  sql`SELECT cause, incident_date AS d FROM rt_incidents WHERE facility_id = ${facilityId} AND cause IS NOT NULL AND btrim(cause) <> ''`,
  sql`SELECT cause, created_at AS d FROM rt_risks WHERE facility_id = ${facilityId} AND cause IS NOT NULL AND btrim(cause) <> ''`,
  ]);
  const allCauseEvents = allCauseEventsRes.flatMap((r) => r.rows).map((r: any) => ({ cause: String(r.cause).trim().toLowerCase(), date: new Date(r.d) }));

function countEventsInRange(causeLower: string, start: Date, end: Date): number {
  return allCauseEvents.filter((e) => e.cause === causeLower && e.date >= start && e.date < end).length;
}

const closedActionsRes = await sql`
SELECT id, cause, description, action, status, created_date, closed_date
FROM rt_actions
WHERE facility_id = ${facilityId} AND cause IS NOT NULL AND btrim(cause) <> '' AND status IN ('completada', 'no_resuelta') AND closed_date IS NOT NULL
`;

const analisisEficacia = closedActionsRes.rows.map((a: any) => {
  const causeLower = String(a.cause).trim().toLowerCase();
  const base = {
    accionId: a.id,
    causa: a.cause,
    descripcion: a.description,
    accionTomada: a.action,
    creada: a.created_date,
    cerrada: a.closed_date,
  };
  if (a.status === "no_resuelta") {
    return {
      ...base,
      eventosAntes: null,
      eventosDespues: null,
      clasificacion: "NO_EFECTIVA",
      clasificacionLabel: EFICACIA_LABELS.NO_EFECTIVA,
      motivo: "La accion fue registrada como no resuelta.",
    };
  }
  const closedDate = new Date(a.closed_date);
  const createdDate = new Date(a.created_date);
  const sinceClosureDays = (now.getTime() - closedDate.getTime()) / 86400000;
  if (sinceClosureDays < 14) {
    return {
      ...base,
      eventosAntes: null,
      eventosDespues: null,
      clasificacion: "NO_EVALUADA",
      clasificacionLabel: EFICACIA_LABELS.NO_EVALUADA,
      motivo: "Accion cerrada hace menos de 14 dias; periodo de observacion insuficiente para evaluar su eficacia.",
    };
  }
  const windowDays = Math.min(sinceClosureDays, 365);
  const beforeStart = addDays(createdDate, -windowDays);
  const afterEndCandidate = addDays(closedDate, windowDays);
  const afterEnd = afterEndCandidate.getTime() < now.getTime() ? afterEndCandidate : now;
  const eventosAntes = countEventsInRange(causeLower, beforeStart, createdDate);
  const eventosDespues = countEventsInRange(causeLower, closedDate, afterEnd);
  let clasificacion = "NO_EVALUADA";
  let motivo = "No se registraron eventos previos con esta causa; no es posible medir una reduccion.";
  if (eventosAntes > 0) {
    if (eventosDespues === 0) {
      clasificacion = "EFECTIVA";
      motivo = "No se registraron nuevos eventos con esta causa desde el cierre de la accion.";
    } else if (eventosDespues < eventosAntes) {
      clasificacion = "PARCIALMENTE_EFECTIVA";
      motivo = "Se redujo la frecuencia de eventos con esta causa, pero continuan ocurriendo.";
    } else {
      clasificacion = "NO_EFECTIVA";
      motivo = "La frecuencia de eventos con esta causa no disminuyo tras el cierre de la accion.";
    }
  }
  return {
    ...base,
    ...
    eventosDespues,
    clasificacion,
    clasificacionLabel: EFICACIA_LABELS[clasificacion] ?? clasificacion,
    motivo,
  };
});

const eficaciaResumen: Record<string, number> = { EFECTIVA: 0, PARCIALMENTE_EFECTIVA: 0, NO_EFECTIVA: 0, NO_EVALUADA: 0 };
  for (const e of analisisEficacia) eficaciaResumen[e.clasificacion] = (eficaciaResumen[e.clasificacion] || 0) + 1;
...
for (const a of alertasCriticasDash) critico.push({ origen: "alerta", detalle: a.message, modulo: a.module });
  ...
  for (const e of analisisEficacia) {
    if (e.clasificacion === "NO_EFECTIVA") importante.push({ origen: "eficacia", detalle: "Accion sobre causa '" + e.causa + "' no fue efectiva", accion: e.accionTomada });
  }
...
const resumenPartes: string[] = [];
  ...
  if (analisisEficacia.length > 0) {
    resumenPartes.push(
      "De " + analisisEficacia.length + " accion(es) cerrada(s) evaluada(s) por eficacia, " + eficaciaResumen.EFECTIVA + " resultaron efectivas, " +
      eficaciaResumen.PARCIALMENTE_EFECTIVA + " parcialmente efectivas y " + eficaciaResumen.NO_EFECTIVA + " no efectivas."
      );
  }
...
return NextResponse.json({
  ...
  analisisEficacia: { items: analisisEficacia, resumen: eficaciaResumen },
  ...
  notasAlcance: [
    ...
    "El analisis de eficacia compara la frecuencia de eventos con la misma causa antes y despues del cierre de cada accion, usando ventanas de tiempo simetricas; se muestra como NO EVALUADA cuando no hay suficiente informacion o tiempo de observacion.",
