 ...
 * API de un resultado puntual de prueba PET (por id).
 *
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const versionChain = await listVersionChain(id);
    return NextResponse.json({ ...record, version_chain: versionChain });
  } catch (error) {
    console.error("Error en GET /api/quality-control/petct/pet-tests/[id]:", error);
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.action === "finalizar") {
      ...
      const updated = await finalizePetTest(id, body.finalized_by);
      if (!updated) {
    ...
    if (body.action === "editar_borrador") {
      const updated = await updateDraftPetTest(id, body);
      if (!updated) {
        return NextResponse.json(
    ...
    if (body.action === "corregir_finalizado") {
      ...
      const corrected = await createCorrectedVersion(id, body.patch ?? {}, body.edit_reason, body.edited_by);
      if (!corrected) {
    ...
    console.error("Error en PUT /api/quality-control/petct/pet-tests/[id]:", error);
