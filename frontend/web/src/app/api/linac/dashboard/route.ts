...
import { ensureMaintenanceExtendedTables, computeMTBF, computeMTTR, computeMaintenanceAvailability } from "@/lib/linac-maintenance";
...
const { rows: radiationByCategory } = await sql`
SELECT category, COUNT(*) AS total,
COUNT(*) FILTER (WHERE status = 'conforme') AS ok_count
FROM linac_radiation_protection
WHERE category IS NOT NULL
GROUP BY category;
`;

const { rows: radiationSemaphore } = await sql`
SELECT semaphore, COUNT(*) AS count
FROM linac_radiation_protection
GROUP BY semaphore;
`;

const { rows: radiationAlerts } = await sql`
...
SELECT maintenance_type, COUNT(*) AS count, COALESCE(SUM(hours), 0) AS total_hours, COALESCE(SUM(cost), 0) AS total_cost, COALESCE(SUM(downtime_hours), 0) AS total_downtime_hours
FROM linac_maintenance GROUP BY maintenance_type;
`;

const { rows: maintenanceSemaphore } = await sql`
SELECT semaphore, COUNT(*) AS count
FROM linac_maintenance
GROUP BY semaphore;
`;

const { rows: maintenanceAlerts } = await sql`
...
(SELECT COUNT(*) FROM linac_radiation_alerts WHERE status = 'abierta') AS radiation_alerts_open,
(SELECT COUNT(*) FROM linac_maintenance_alerts WHERE status = 'abierta') AS maintenance_alerts_open,
(SELECT COUNT(*) FROM linac_maintenance WHERE maintenance_type = 'correctivo') AS corrective_count,
(SELECT COALESCE(SUM(downtime_hours), 0) FROM linac_maintenance WHERE maintenance_type = 'correctivo') AS corrective_downtime_hours,
...
const correctiveCount = Number(t.corrective_count || 0);
const correctiveDowntimeHours = Number(t.corrective_downtime_hours || 0);
const mtbf = computeMTBF(operatingHours, correctiveCount);
const mttr = computeMTTR(correctiveDowntimeHours, correctiveCount);
const maintenanceAvailability = computeMaintenanceAvailability(mtbf, mttr);
...
radiationAlertsOpen: Number(t.radiation_alerts_open || 0),
maintenanceAlertsOpen: Number(t.maintenance_alerts_open || 0),
