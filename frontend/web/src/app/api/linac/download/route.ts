...
maintenance_photo: "linac_maintenance",
maintenance_document: "linac_maintenance",
...
beam_data: "linac_beam_data",
radiation: "linac_radiation_protection",
};
...
else if (table === "maintenance_photo") rows = (await sql`SELECT photo_file_name AS file_name, photo_blob_url AS blob_url, photo_mime_type AS mime_type FROM linac_maintenance WHERE id = ${id}`).rows;
else if (table === "maintenance_document") rows = (await sql`SELECT document_file_name AS file_name, document_blob_url AS blob_url, document_mime_type AS mime_type FROM linac_maintenance WHERE id = ${id}`).rows;
...
else if (table === "beam_data") rows = (await sql`SELECT file_name, blob_url, mime_type FROM linac_beam_data WHERE id = ${id}`).rows;
else if (table === "radiation") rows = (await sql`SELECT file_name, blob_url, mime_type, size_bytes FROM linac_radiation_protection WHERE id = ${id}`).rows;
