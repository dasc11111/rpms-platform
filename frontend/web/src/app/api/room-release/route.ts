...
export async function POST(req: NextRequest) {
  ...
  const { rows } = await sql`
    INSERT INTO room_release_records (
      release_date, admission_date, service, sala, room_number, ubicacion, paciente_nombre, paciente_run,
      ...
      puntos_medicion, created_by
    ) VALUES (
      ${release_date}, ${body.admission_date ?? null}, ${service}, ${sala}, ${body.room_number ?? null}, ${ubicacion}, ${paciente_nombre}, ${body.paciente_run ?? null},
      ...
      ${puntosMedicion ? JSON.stringify(puntosMedicion) : null}, ${body.created_by ?? null}
    )
    RETURNING *
  `;
